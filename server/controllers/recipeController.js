const Recipes=require('../models/databse');
...
exports.exploreCategoriesById = async(req, res) => { 
  try {
    ...
    console.log("category info:", categoryById)
...
exports.searchRecipe = async (req, res) => {
  try {
    ...
    let uniqueRecipes = recipes.reduce((acc, recipe) => {
      if (!acc.some((r) => r.name === recipe.name)) {
        acc.push(recipe);
      }
      return acc;
    }, []);

    console.log("the recipes are: ", uniqueRecipes);
    res.render("search", { title: "Cooking Blog - Search", recipe: uniqueRecipes });
  } catch (error) {
...
exports.exploreRandom = async(req, res) => {
  try {
    ...
    res.render('explore-random', { title: 'Cooking Blog - Explore Latest', recipes } );
  } catch (error) {
    res.status(500).send({message: error.message || "Error Occured" });
  }
} 
...
exports.submitRecipeOnPost = async (req, res) => {
  try {
    ...
    // Handle the error (you can log it or use req.flash)
...
/*  async function deleteRecipe(){
   try {
     await Recipe.deleteOne({ name: 'sdeve' });
   } catch (error) {
     console.log(error);
   }
 }
